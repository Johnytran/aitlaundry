...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_orderlogbook/assets/css/orderlogbook.css');
$document->addStyleSheet(JUri::root() . 'media/com_orderlogbook/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_orderlogbook');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_orderlogbook&task=orderlogbooks.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'orderlogbookList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="orderlogbookList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_orderlogbook');
					$canEdit    = $user->authorise('core.edit', 'com_orderlogbook');
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'orderlogbooks.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
