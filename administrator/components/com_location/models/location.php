...
class LocationModelLocation extends JModelAdmin
{
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_location.location';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_location.location', 'location',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_location'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__location');
				$max             = $db->loadResult();
